(function () {
  ...
  function setMcuConfiguration(configuration) {
    ...
    if (schematicState.initialized) {
      ...
      if (!configuration.preserveView && !schematicState.userView) requestAnimationFrame(function () { fitToContent(false); });
    }
  ...
  function requestSpiceProbeValidation(force) {
    var request = window.AlicePlatform && window.AlicePlatform.fetch || window.fetch;
    ...
    return request("/api/spice-solve", {
      method: "POST",
  ...
  function applyProjectModel(model) {
    ...
    // Firmware builds used to draw an LED branch for every GPIO output. Clean
    // up those legacy branches, but never synthesize new output circuitry.
  ...
  function updateWireCancelAction() {
    if (!schematicNodes.cancelWireButton) return;
    schematicNodes.cancelWireButton.hidden = !schematicState.wireStart;
  }

  function cancelWire() {
    schematicState.wireStart = null;
    schematicState.previewPoint = null;
    schematicState.selectPinGesture = null;
    schematicState.componentElements.forEach(function (element) {
      element.querySelectorAll(".component-pin.wire-start").forEach(function (pin) { pin.classList.remove("wire-start"); });
    });
    updateWireCancelAction();
    renderWires();
  }
  ...
  function setTool(tool) {
    ...
    else setStatus("选择工具 · 双击端子开始连接 · 拖动导线调整 · 双击线段增加拐点");
  }
  ...
  function rememberSelectPinGesture(pinElement) {
    schematicState.selectPinGesture = {
      endpoint: endpointKey(endpointFromPin(pinElement)),
      at: Date.now()
    };
  }

  function isRecentSelectPinGesture(pinElement) {
    var gesture = schematicState.selectPinGesture;
    return Boolean(gesture && gesture.endpoint === endpointKey(endpointFromPin(pinElement)) && Date.now() - gesture.at < 650);
  }

  function handleWirePin(pinElement, options) {
    ...
    if (!schematicState.wireStart) {
      ...
      updateWireCancelAction();
      setStatus("起点 " + pinElement.title + " · " + (options && options.selectMode ? "单击另一个端子完成连接" : "请选择终点") + " · Esc 取消");
      renderWires();
      return "started";
    }
  ...
  function bindEvents() {
    ...
    schematicNodes.components.addEventListener("pointerdown", function (event) {
      ...
      if (pin && schematicState.tool === "wire" && event.button === 0) {
        event.preventDefault();
        event.stopPropagation();
        handleWirePin(pin);
        return;
      }
      if (pin && schematicState.tool === "select" && event.button === 0) {
        event.preventDefault();
        event.stopPropagation();
        var pinComponent = pin.closest(".schematic-component");
        if (pinComponent) selectEntity("component", pinComponent.dataset.componentId);
        if (isRecentSelectPinGesture(pin)) return;
        if (schematicState.wireStart) {
          handleWirePin(pin, { selectMode: true });
          rememberSelectPinGesture(pin);
        }
        return;
      }
    ...
    schematicNodes.components.addEventListener("dblclick", function (event) {
      ...
      if (pin && schematicState.tool === "select" && event.button === 0) {
        ...
        event.stopPropagation();
        cancelActiveInteraction(false);
        if (isRecentSelectPinGesture(pin)) {
          schematicState.selectPinGesture = null;
          return;
        }
        handleWirePin(pin, { selectMode: true });
    ...
    schematicNodes.viewport.addEventListener("pointerdown", function (event) {
      ...
      if (schematicState.tool === "wire" && event.button === 2) {
        event.preventDefault();
        cancelActiveInteraction(false);
        if (schematicState.wireStart) {
          cancelWire();
          setStatus("当前导线已取消 · 仍处于导线模式");
        } else {
          setTool("select");
          setStatus("已退出导线模式");
        }
    ...
    if (schematicNodes.cancelWireButton) schematicNodes.cancelWireButton.addEventListener("click", function () {
      cancelWire();
      setStatus("当前导线已取消");
      schematicNodes.viewport.focus({ preventScroll: true });
    });
  ...
  function initializeSchematic() {
    ...
    schematicNodes.cancelWireButton = document.getElementById("cancelWireButton");
